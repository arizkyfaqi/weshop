<?php
	...
	function rupiah($niali = 0){
		$string = "Rp,".number_format($niali);
		return $string;
	}
	...
	function admin_only($module, $level){
		if ($level != "superadmin") {
			$admin_page = array("kategori", "barang", "kota", "user", "banner");
	...
	function pagination ($query, $data_per_halaman, $pagination, $url){
		...
		if ($total_halaman >= $batasJumblahHalaman) {
			
			if ($pagination > $batasPosisiNomor) {
				$mulaiPagination = $pagination - ($batasPosisiNomor - 1);
			}

			$batasAkhirPagination = ($mulaiPagination - 1) + $batasJumblahHalaman;
